...
 	1  Download EqualizerAPO Linked:		->	 (https://equalizerapo.com/)
 	(and select the correct audio device)
...
 	    Right-click "ABI_AUDIOSWITCH.exe" → Properties → Compatibility → enable "Run this program as an administrator" right click the .exe again → "Run as administrator"
	...
	12. In the Config Folder is an "AHK_LOG.txt" meant as a Status Log/debugging tool. It will show you if it detects the Game opened/closed and if it changes the Preset EQ. 
		(note! do not change Filenames or anything else unless expressed or you know what you are doing. If the it's not working, try restart PC or run .exe as administrator again. Otherwise go through the checklist again.)
...
 	--	explanation on how to ajust or how everything works See the explanation folder or you can inspect the ahk File to insptect the code!					--
...
 	13. Go to Crinicles Website for IEMS:	    →"https://graph.hangout.audio/iem/5128/?share=JM-1_Target,Daybreak&bass=0&tilt=-1&treble=0&ear=0" →
 				    for headphones: →"https://graph.hangout.audio/headphones/?share=KEMAR-DF_(KB50XX)_Target,HD600_S3_(2020)_(fresh_pads)&bass=4&tilt=-1&treble=0&ear=0"

 	    note! this process can get quite wonky, I reccomend to watch crinacle's video! Link:(https://www.youtube.com/watch?v=PsijHAVLbuY)


	    enter the name and brand(or just the brand) of your headphones in the searchbar, select them, switch over to Equalizer and hit autoEQ, then if your satisfied hit "Export Parametric EQ"
	    and download the .txt File, it should look like something similar to this↓	     copy it! and paste it into the "HEADPHONES_EQ.txt" File inside the config. 
